...
`;
		...
		function apply(ctx) {
			...
			// 客户端会话存储已被 agent-preset/selected → noteAgentPreset 更新，
			// 直接读本地状态改标签，无需 wire 请求、快速连点也不丢。
			const applyLocalSessionPreset = () => {
				if (disposing) return;
				const cur = currentSession();
				if (cur && cur.blank && cur.agentPreset) {
					if (state.options.length === 0 || state.options.some((o) => o.id === cur.agentPreset)) {
						if (state.current !== cur.agentPreset) {
							state.current = cur.agentPreset;
							state.error = null;
							renderLabel();
			...
			const select = async (id) => {
				...
				renderLabel();
				try {
					// 先应用到当前空白会话（徽章 select 同款，await 等 agent-preset/selected
					// 落地、会话存储更新），再写默认——保证徽章因 settings 变更而 reload 时
					// 读到的会话预设已是新值，消除"第一次切换徽章不变、之后慢一拍"的竞态。
					const cur = currentSession();
					if (cur && cur.blank) {
						try {
			...
			try {
				...
				if (remote && typeof remote.$on === "function") {
					...
					disposers.push(remote.$on("agent-preset/selected", () => {
						applyLocalSessionPreset();
