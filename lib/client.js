window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = `
.dsh-smode-inline{display:inline-block;flex:0 1 auto;min-width:0;max-width:104px;margin-right:3px;padding:0 4px;border-radius:6px;font-size:12px;line-height:1.6;color:var(--dsw-alias-label-secondary,#9aa0a6);opacity:.8;cursor:pointer;user-select:none;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;transition:background .15s ease,color .15s ease,opacity .15s ease}
/* 宿主「新会话」文字标签不可收缩：空间不足时由上方徽章（可收缩+省略号）让步，
   「新会话」三个字永远完整；属性选择器与挂载锚点 [class*="newSession"] 同理（CSS module 哈希无关） */
[class*="newSessionLabel"]{flex-shrink:0}
...
.dsh-smode-menu-item .dsh-smode-name{display:block;white-space:normal;overflow-wrap:anywhere}
